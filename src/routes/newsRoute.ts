  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!title.en || !title.ar || !title.fr) {
        return res
          .status(400)
          .json({ message: "Title required in all languages" });
      }
      if (!paragraph.en || !paragraph.ar || !paragraph.fr) {
        return res
          .status(400)
          .json({ message: "Paragraph required in all languages" });
      }
      ...
      if (!image && !youtubeLink) {
        return res
          .status(400)
          .json({ message: "Media required (image or YouTube)" });
      }
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const title = req.body.title
        ? parseMultilingualField(req.body.title, "title")
        : existing.title;
      const paragraph = req.body.paragraph
        ? parseMultilingualField(req.body.paragraph, "paragraph")
        : existing.paragraph;
      ...
      if (!image && !youtubeLink) {
        return res
          .status(400)
          .json({ message: "Media required (image/YouTube)" });
      }
